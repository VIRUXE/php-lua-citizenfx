...
ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_get_version, 0, 0, 0)
ZEND_END_ARG_INFO()
/* }}} */
...
zend_object *php_lua_create_object(zend_class_entry *ce)
{
	...
	intern = (php_lua_object*) zend_object_alloc(sizeof(php_lua_object), ce);
	intern->L = L;
...
/** {{{ static zval * php_lua_read_property(zend_object *object, zend_string *member, int type, void **cache_slot, zval *rv)
*/
zval *php_lua_read_property(zend_object *object, zend_string *member, int type, void **cache_slot, zval *rv){
	...
	zval tmp;
	ZVAL_OBJ(&tmp, object);
	php_lua_get_zval_from_lua(L, -1, &tmp, rv);
	lua_pop(L, 1);
	return rv;
}
/* }}} */

/** {{{ static zval * php_lua_write_property(zend_object *object, zend_string *member, zval *value, void **cache_slot)
*/
static zval * php_lua_write_property(zend_object *object, zend_string *member, zval *value, void **cache_slot) {
	lua_State *L = (php_lua_obj_from_obj(object))->L;
...
static int php_lua_call_callback(lua_State *L) {
	...
	if (!zend_is_callable(func, 0, NULL)) {
		...
		params = (zval *) safe_emalloc(sizeof(zval), arg_num, 0);
		for (i = 0; i < arg_num; i++) {
...
zval *php_lua_get_zval_from_lua(lua_State *L, int index, zval *lua_obj, zval *rv) /* {{{ */ {
	switch (lua_type(L, index)) {
		...
		case LUA_TVECTOR:
			{
				array_init(rv);
				struct lua_CFloat4 { float x, y, z, w; } f4;
				// The mangled header defines lua_tovector as returning void*, so we cast it to int.
				int variant = (int)(intptr_t)lua_tovector(L, index, 0, &f4);
				
				// LUA_VVECTOR2 = LUA_TVECTOR | (0 << 4) = 4
				// LUA_VVECTOR3 = LUA_TVECTOR | (1 << 4) = 20
				// LUA_VVECTOR4 = LUA_TVECTOR | (2 << 4) = 36
				// LUA_VQUAT    = LUA_TVECTOR | (3 << 4) = 52
				
				if (variant == 4) { // VVECTOR2
					add_assoc_double(rv, "x", (double)f4.x);
					add_assoc_double(rv, "y", (double)f4.y);
				} else if (variant == 20) { // VVECTOR3
					add_assoc_double(rv, "x", (double)f4.x);
					add_assoc_double(rv, "y", (double)f4.y);
					add_assoc_double(rv, "z", (double)f4.z);
				} else if (variant == 36 || variant == 52) { // VVECTOR4 / QUAT
					add_assoc_double(rv, "x", (double)f4.x);
					add_assoc_double(rv, "y", (double)f4.y);
					add_assoc_double(rv, "z", (double)f4.z);
					add_assoc_double(rv, "w", (double)f4.w);
				} else {
					// Fallback
					add_assoc_double(rv, "x", (double)f4.x);
					add_assoc_double(rv, "y", (double)f4.y);
		...
		case LUA_TMATRIX:
			{
				array_init(rv);
				// Matrices are basically array of vectors. Since we don't know the exact size easily
				// without C++ glm headers, we will just return an empty array for now or a basic representation.
				// A simple empty array is safer than a crash.
			}
			break;
		case LUA_TUSERDATA:
		case LUA_TTHREAD:
...
zend_function_entry lua_class_methods[] = {
	PHP_ME(lua, __construct,		arginfo_lua_construct,	ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
	...
	PHP_ME(lua, getVersion,			arginfo_lua_get_version,ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
