...
PHP 8+ bindings for the FiveM LuaGLM runtime (citizenfx/lua). 
This extension enables native parsing of vectors, quaternions, and Jenkins hashes directly from PHP without memory corruption or segmentation faults.

### Features
- Native support for `LUA_TVECTOR` and `LUA_TMATRIX` types introduced by LuaGLM.
- Completely stable under PHP-FPM (no teardown crashes or GPFs).
- Translates `vec2`, `vec3`, `vec4`, and `quat` directly into PHP associative arrays.
- Backwards compatible with standard Lua 5.4.

### Requirements
- PHP 8.0+
- citizenfx/lua headers (LuaGLM 5.4.4)

### Building
```bash
phpize
./configure --with-lua-dir=/path/to/citizen-lua-dist
make -j$(nproc)
sudo make install
```
