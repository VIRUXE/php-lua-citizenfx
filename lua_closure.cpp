...
ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_closure_construct, 0, 0, 0)
ZEND_END_ARG_INFO()

/* {{{ lua_class_methods[]
 */
zend_function_entry lua_closure_methods[] = {
	PHP_ME(lua_closure, __construct,		arginfo_lua_closure_construct,	ZEND_ACC_PRIVATE|ZEND_ACC_CTOR)
...
static void php_lua_closure_free_obj(zend_object *zobj) /* {{{ */
{
	...
	zval_ptr_dtor(&(objval->lua));
	zend_object_std_dtor(zobj);
} /* }}} */

zend_object *php_lua_closure_create_object(zend_class_entry *ce) /* {{{ */
{
	lua_closure_object *objval = (lua_closure_object *) zend_object_alloc(sizeof(lua_closure_object), ce);

	zend_object_std_init(&objval->std, ce);
	object_properties_init(&objval->std, ce);
	objval->std.handlers = &lua_closure_handlers;
